...
//////////////Enable Switches///////////////
if (isNil "aet_aux_ffpp_enabled") then { aet_aux_ffpp_enabled = true; };
if (isNil "aet_aux_ffpp_tellInstigator") then { aet_aux_ffpp_tellInstigator = false; };
if (isNil "aet_aux_ffpp_tellVictim") then { aet_aux_ffpp_tellVictim = false; };
if (isNil "aet_aux_ffpp_tellAdmin") then { aet_aux_ffpp_tellAdmin = true; };
...
private _notifyVictim = {
    if (!aet_aux_ffpp_tellVictim) exitWith {};
...
private _notifyInstigator = {
    ...
    if (!aet_aux_ffpp_tellInstigator) exitWith {};
...
private _notifyAdmin = {
    ...
    if (!aet_aux_ffpp_tellAdmin) exitWith {};
...
_exemption = switch (true) do {  // ~0.012 ms for all false cases
    case (!aet_aux_ffpp_enabled):                  {"FF PUNISH IS DISABLED"};
